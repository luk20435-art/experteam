...
export default function AddWorkOrder() {
  ...
  // โหลดข้อมูลจาก localStorage ครั้งเดียว (client-side เท่านั้น)
  ...
  useEffect(() => {
    const loadData = () => {
      ...
      if (r) {
        try {
          setRequests(JSON.parse(r));
        } catch (e) {
          console.error("Failed to parse work-requests", e);
        }
      }
  ...
  // ตั้งค่าเลข WO อัตโนมัติตอนโหลด
  useEffect(() => {
    setForm((prev) => ({ ...prev, orderNumber: generateNumber() }));
  }, [generateNumber]);

  const handleSave = () => {
    if (!form.workRequestId || !form.assignedTo || !form.startDate) {
      alert("กรุณากรอกข้อมูลที่จำเป็น: คำขอ, ผู้รับงาน, วันที่เริ่ม");
      return;
    }
    ...
    const newId = Math.max(...saved.map((o) => o.id), 0) + 1;
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>ข้อมูลคำสั่งงาน</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* คำขอ */}
          <div className="space-y-2">
            <Label>
              คำขอที่เกี่ยวข้อง <span className="text-red-500">*</span>
            </Label>
            <Select
          ...
          <div className="space-y-2">
            <Label>
              ผู้รับงาน <span className="text-red-500">*</span>
            </Label>
            <Select
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm({ ...form, estimatedHours: +e.target.value || 0 })
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm({ ...form, actualHours: +e.target.value || 0 })
                }
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm({ ...form, materialsCost: +e.target.value || 0 })
                }
              />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={form.status}
              onValueChange={(v) =>
                setForm({ ...form, status: v as "Draft" | "Approved" | "In Progress" | "Completed" | "Cancelled" })
              }
            >
              <SelectTrigger>
                <SelectValue />
