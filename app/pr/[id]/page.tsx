...
import { ArrowLeft, Pencil, FileDown, Calendar, User, Building2, Target, Clock, Package } from "lucide-react"
import { useData } from "@/src/contexts/data-context"
import { formatCurrency, formatDate, formatDateTime, getStatusColor, getStatusLabel } from "@/src/lib/utils"
...
export default function PRDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <div className="flex min-h-[50vh] items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <div className="text-xl font-semibold text-gray-700">ไม่พบข้อมูล PR</div>
          <Link href="/pr">
  ...
  const handleExportPDF = () => {
    ...
    <head>
      <meta charset="utf-8">
      <title>ใบสั่งซื้อ ${pr.prNumber}</title>
    ...
    <body>
      ...
      <div class="info-grid">
        <div class="info-row">
          <div class="info-cell" style="width: 50%">
            <div style="margin-bottom: 4px"><span class="info-label">งาน Job:</span></div>
            <div>${pr.title || '-'}</div>
          </div>
        ...
        <div class="info-row">
          <div class="info-cell">
            <div class="info-label">Ref. No./ใบเสนอราคา</div>
            <div>-</div>
          </div>
          <div class="info-cell section-header" colspan="2">
            ใบขอซื้อ<br>PURCHASE REQUISITION
          </div>
        </div>
        
        <div class="info-row">
          <div class="info-cell">
            <div class="info-label">สถานที่ ( Location )</div>
        ...
        <div class="info-row">
          ...
          <div class="info-cell">
            <div class="info-label">Purchase by :</div>
      ...
      <table class="items-table">
        ...
        <tbody>
          ...
          `).join('')}
          ${Array.from({ length: Math.max(0, 5 - pr.items.length) }).map(() => `
            <tr>
              <td>&nbsp;</td>
              <td class="left">&nbsp;</td>
              <td>&nbsp;</td>
      ...
      <div class="total-section">
        <div class="total-row">
          ...
          <span>${formatCurrency(pr.subtotal || 0)}</span>
        </div>
        <div class="total-row">
          <span>VAT (${pr.vatRate ?? 0}%)</span>
          <span>${formatCurrency(pr.vatAmount || 0)}</span>
        </div>
        <div class="total-row">
          <span>Service Tax (${pr.serviceTaxRate ?? 0}%)</span>
          <span>${formatCurrency(pr.serviceTaxAmount || 0)}</span>
        </div>
        <div class="total-row grand-total">
          <strong>รวมทั้งสิ้น</strong>
          <strong>${formatCurrency(pr.totalAmount || 0)}</strong>
        </div>
      </div>
      ...
      <div class="note-section">
        ...
        <div class="note-box">
          สั่งสินค้าภายในวันที่: 4-5 working day<br>
          เงื่อนไขการชำระเงิน: เครดิต 30 days
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <Badge className={`${getStatusColor(pr.status)} text-sm px-3 py-1`}>
                    {getStatusLabel(pr.status)}
                  </Badge>
                </div>
                <p className="text-lg text-slate-600">{pr.title}</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <CardContent className="p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-500 mb-1">แผนก</div>
                  <div className="text-lg font-semibold text-slate-900 truncate">{pr.department}</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-sm border-slate-200 hover:shadow-md transition-shadow">
            <CardContent className="p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-500 mb-1">ผู้ขอ</div>
                  <div className="text-lg font-semibold text-slate-900 truncate">{pr.requestedBy}</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-sm border-slate-200 hover:shadow-md transition-shadow">
            <CardContent className="p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-500 mb-1">วันที่ขอ</div>
                  <div className="text-lg font-semibold text-slate-900">{formatDate(pr.requestDate)}</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-sm border-slate-200 hover:shadow-md transition-shadow">
            <CardContent className="p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-500 mb-1">วันที่ต้องการ</div>
                  <div className="text-lg font-semibold text-slate-900">{formatDate(pr.requiredDate)}</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {/* Purpose */}
          <Card className="shadow-sm border-slate-200 hover:shadow-md transition-shadow md:col-span-2">
            <CardContent className="p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-500 mb-1">วัตถุประสงค์</div>
                  <div className="text-lg font-semibold text-slate-900">{pr.purpose}</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <CardContent className="p-6">
            {Array.isArray(pr.items) && pr.items.length > 0 ? (
                    ...
                    <tbody className="divide-y divide-slate-200">
                      {pr.items.map(item => (
                        <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-4 py-3 font-medium text-slate-600">{item.itemNo}</td>
                          <td className="px-4 py-3">
                            <div className="font-medium text-slate-900">{item.description}</div>
                            {item.remarks && (
                              <div className="text-sm text-slate-500 mt-1">{item.remarks}</div>
                            )}
                          </td>
                      ...
                      <CardContent className="p-4 space-y-3">
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <div className="text-xs text-slate-500 mb-1">ลำดับ {item.itemNo}</div>
                        ...
                        <div className="grid grid-cols-2 gap-3 pt-3 border-t">
                          ...
                          <div className="text-right">
                            ...
                            <div className="font-medium">{formatCurrency(item.estimatedPrice)}</div>
                          </div>
                        </div>
                ...
                <div className="bg-gradient-to-br from-slate-50 to-blue-50 rounded-xl p-6 border border-slate-200">
                  <div className="space-y-3">
                    <div className="flex justify-between items-center text-slate-700">
                      ...
                      <span className="text-lg font-semibold">{formatCurrency(pr.subtotal || 0)}</span>
                    </div>
                    <div className="flex justify-between items-center text-slate-700">
                      <span className="font-medium">ภาษีมูลค่าเพิ่ม (VAT {pr.vatRate ?? 0}%)</span>
                      <span className="text-lg font-semibold">{formatCurrency(pr.vatAmount || 0)}</span>
                    </div>
                    <div className="flex justify-between items-center text-slate-700">
                      <span className="font-medium">ภาษีบริการ ({pr.serviceTaxRate ?? 0}%)</span>
                      <span className="text-lg font-semibold">{formatCurrency(pr.serviceTaxAmount || 0)}</span>
                    </div>
              ...
              <div className="text-center py-12">
                ...
                <div className="text-lg font-medium text-slate-600">ไม่มีรายการสินค้า</div>
              </div>
            )}
          </CardContent>
        </Card>
